...
def _check_workspace_boundary(path: Path) -> None:
    ...
    raise WorkspaceBoundaryError(
        str(resolved_path), [str(d) for d in config.security.workspace_dirs]
    )
...
def _check_file_size(path: Path) -> None:
    ...
    if file_size > max_size:
        raise FileTooLargeError(str(path), file_size, max_size)
...
def validate_pptx_path(pptx_path: str | Path) -> Path:
    ...
    if path.suffix.lower() not in config.security.allowed_extensions:
        raise InvalidPathError(
            str(path), f"Invalid file extension. Allowed: {config.security.allowed_extensions}"
        )
...
def validate_slide_number(slide_number: int, max_slides: int) -> int:
    ...
    if not isinstance(slide_number, int):
        raise ValidationError(f"Slide number must be an integer, got {type(slide_number).__name__}")
...
def validate_image_path(image_path: str | Path) -> Path:
    ...
    if path.suffix.lower() not in valid_extensions:
        raise InvalidPathError(
            str(path), f"Unsupported image format: {path.suffix}. Supported: {valid_extensions}"
        )
...
def parse_slide_range(slide_range: str) -> List[int]:
    ...
    try:
        ...
        raise ValueError(
            f"Invalid slide range numbers: {slide_range!r}. Both values must be integers."
        ) from e
...
def validate_slide_numbers(slide_numbers: List[int], max_slides: int) -> List[int]:
    ...
    for slide_num in slide_numbers:
        if not isinstance(slide_num, int):
            raise ValueError(
                f"All slide numbers must be integers, got {type(slide_num).__name__}: {slide_num!r}"
            )
...
def validate_batch_updates(updates: List[dict], max_slides: int) -> List[dict]:
    ...
    for i, update in enumerate(updates):
        if not isinstance(update, dict):
            raise ValueError(
                f"Update at index {i} must be a dictionary, got {type(update).__name__}"
            )
        ...
        if not isinstance(slide_num, int):
            raise ValueError(
                f"Update at index {i}: slide_number must be integer, got {type(slide_num).__name__}"
            )

        if not isinstance(notes_text, str):
            raise ValueError(
                f"Update at index {i}: notes_text must be string, got {type(notes_text).__name__}"
            )
