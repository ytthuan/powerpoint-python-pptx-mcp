...
class TokenBucketRateLimiter:
    ...
    def __init__(self, rate: int, per: float = 60.0, burst: Optional[int] = None):
        ...
        logger.info(
            f"Rate limiter initialized: {rate} requests per {per} seconds " f"(burst: {self.burst})"
        )
    ...
    async def acquire(self, tokens: int = 1) -> None:
        ...
        if tokens > self.burst:
            raise ValueError(f"Requested {tokens} tokens exceeds burst limit of {self.burst}")
...
class RateLimiterMiddleware:
        ...
        if limiter is None:
            ...
            if config.performance.enable_rate_limiting:
                self.limiter = TokenBucketRateLimiter(
                    rate=config.performance.max_requests_per_minute, per=60.0
                )
            else:
                self.limiter = None
        else:
    ...
    async def __call__(self, name, args, next_handler):
        ...
        if not acquired:
            ...
            logger.warning(
                f"Rate limit exceeded for tool: {name}. " f"Retry after {wait_time:.2f}s"
            )
